...
test("all personal archive downloads match the shared content, including the UTF-8 BOM", async () => {
  for (const record of content.records) {
    assert.equal(
      (
        await readFile(
          new URL(
            `../public/archives/ZIHAN-${record.id}.txt`,
            import.meta.url,
          ),
          "utf8",
        )
...
test("rejects executable link protocols in personal contact links", () => {
  const edited = structuredClone(content);
  edited.records[0].links = [{ label: "contact", url: "javascript:alert(1)" }];
  assert.throws(() => validateContent(edited), /安全的网页或邮件链接/);
});
