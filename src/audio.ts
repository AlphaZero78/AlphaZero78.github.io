...
function typingSample(c: BaseAudioContext) {
  ...
  if (!bank) {
    bank = {
      buffers: [880, 1046, 1174].map((frequency) => {
        const buffer = c.createBuffer(1, Math.floor(c.sampleRate * .038), c.sampleRate);
        const data = buffer.getChannelData(0);
        for (let i = 0; i < data.length; i++) {
          const t = i / c.sampleRate;
          const envelope = Math.min(1, t / .001) * Math.exp(-t * 150) * Math.min(1, (.038 - t) / .003);
          data[i] = .23 * envelope * (Math.sin(2 * Math.PI * frequency * t) + .2 * Math.sin(2 * Math.PI * frequency * 2.1 * t));
        }
        return buffer;
      }),
      next: 0,
