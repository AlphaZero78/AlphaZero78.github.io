...
import { fileAtSlot, fileLocation, columnFiles } from "./data";
import {
  cellKey,
  sameCell,
  selectionCell,
...
export class ArchiveScene {
  ...
  private quality = normalizeQuality(qualityPresets.performance);
  ...
  constructor(
    ...
    this.renderer.domElement.setAttribute(
      "aria-label",
      "个人档案阵列，点击选择，左右拖动切换基本介绍、教育背景、项目经历、校园经历和联系方式",
    );
  ...
  get navigationReadiness() {
    const chosen = this.cellPosition(this.selectedCell);
    return {
      closed: !this.targetDetail && this.returnY === null && this.lift.value < .43 &&
        Math.abs(this.lift.velocity) < .1 && this.detail < .03,
      aligned: Math.abs(this.columnCamera.value - chosen.x) < .08 &&
        Math.abs(this.columnCamera.velocity) < .15 &&
        Math.abs(this.rail.value - (-2.17 - chosen.z)) < .05 &&
        Math.abs(this.rail.velocity) < .1 &&
        Math.abs(this.laneFocus.value - this.selectedCell.lane) < .03,
      opened: Boolean(this.targetDetail) && this.detailVisibility > .99 &&
