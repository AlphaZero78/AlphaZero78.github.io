...
export function pwaSettingsMarkup() {
  const status = !import.meta.env.PROD || localPreview ? "本地预览直接加载最新文件，不保存离线副本。"
...
export async function initPwa(notify: (message: string) => void) {
  tell = notify;
  if (started || localPreview || !import.meta.env.PROD || !window.isSecureContext || !("serviceWorker" in navigator)) return;
  started = true;
  try {
...
document.addEventListener("click", async event => {
  ...
  if (button.dataset.pwaAction === "update" && registration?.waiting) {
    ...
    registration.waiting.postMessage({ type: "ALPHAZERO_APPLY_UPDATE" });
  }
