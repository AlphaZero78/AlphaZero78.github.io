...
import { redactionProgress } from "./redaction-timing";
...
export class DocumentDecryption {
  ...
  update(now: number, frame: DecryptionFrame, reduced: boolean) {
    ...
    if (reduced) this.progress = 1;
      ...
      // Start soon after the extracted document is ready. The text no longer
      // waits for the whole glass scan; its original reveal motion is retained.
      if (this.started === null && frame.phase !== "waiting") this.started = now;
      if (this.started !== null)
        this.progress = redactionProgress(now - this.started);
    }
