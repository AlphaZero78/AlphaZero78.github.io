...
$("#stage").innerHTML = `
  ...
  <nav class="system-nav" aria-label="系统导航">
    <button class="profile-nav" data-action="profile" title="个人概览"><span class="profile-glyph" aria-hidden="true">◧</span> 个人概览</button>
  ...
  <section id="boot" class="boot" aria-label="系统启动">
    ...
    <div class="scan"><svg viewBox="0 0 1920 1080" aria-hidden="true"><g fill="none" stroke="#080a08" stroke-width="2" stroke-linecap="round"><path/><path stroke="#fff"/><path/><path/><path/><path/><circle class="orbit-dot" r="8" fill="#ed821b" stroke="none"/><circle class="orbit-dot" r="8" fill="#ed821b" stroke="none"/><circle class="scan-core" cx="960" cy="540" r="5" fill="#080a08" stroke="none"/></g></svg><span>ARCHIVE READY</span></div>
    <div class="welcome"><div class="welcome-panel"></div><div class="welcome-heading">WELCOME TO</div><div class="welcome-company"><strong>ZIHAN DONG.</strong><strong class="welcome-highlight" aria-hidden="true">ZIHAN DONG.</strong></div><div class="welcome-database">PERSONAL ARCHIVE</div><div class="welcome-logo">${logo}</div></div>
  </section>
  ...
  <section id="detail-ui" class="detail-ui" aria-label="档案内容" hidden>
    ...
    <div class="object-caption"><span id="object-id">NO.001</span><div>PERSONAL ARCHIVE</div><small>拖动查看 · 滚轮切换档案</small><button class="viewer-open" data-action="model-viewer">360° 查看文档模型 <span>↗</span></button></div>
    <article id="detail-content" class="detail-content"></article>
  </section>
  <div class="powered">PERSONAL ARCHIVE <b> / ONLINE</b><i></i></div>
...
const requestedIndex = records.findIndex(r => r.id === reviewParams.get("file"));
...
function setMode(next: Mode) {
  renderBudget.wake(performance.now());
...
function stepFile(direction: number) {
  ...
  if (files.length < 2) { stepColumn(direction); return; }
  select(
...
function updateSelection(navigation?: ArchiveNavigation) {
  ...
  $(".count-total").textContent = String(records.length).padStart(2, "0");
...
function syncJourney() {
  $("#stage").dataset.journey = archiveJourney.phase;
  $("#three-scene").inert = archiveJourney.active;
}
function cancelJourney() {
  archiveJourney.cancel(performance.now());
  syncJourney();
}

let detailWheelTotal = 0;
let detailWheelTime = 0;
function textCanScroll(target: EventTarget | null, delta: number) {
  let element = target instanceof Element ? target : null;
  // Preserve nested text scrolling; only advance at the reading panel's boundary.
  if (!element?.closest("#detail-content")) return false;
  while (element && element.id !== "stage") {
    const html = element as HTMLElement;
    if (/auto|scroll/.test(getComputedStyle(html).overflowY) && html.scrollHeight > html.clientHeight + 1) {
      if (delta > 0 && html.scrollTop + html.clientHeight < html.scrollHeight - 2) return true;
      if (delta < 0 && html.scrollTop > 2) return true;
    }
...
document.addEventListener("wheel", (event) => {
  if (!started || !ready || modal || viewer?.isOpen || event.ctrlKey ||
      Math.abs(event.deltaX) > Math.abs(event.deltaY) || !event.deltaY) return;
  if (!archiveJourney.active && mode !== "detail") return;
  if (!archiveJourney.active && textCanScroll(event.target, event.deltaY)) {
    detailWheelTotal = 0;
    return;
  }
  ...
  if (archiveJourney.active) { detailWheelTotal = 0; detailWheelTime = now; return; }
  const delta = event.deltaY * (event.deltaMode === 1 ? 40 : event.deltaMode === 2 ? innerHeight : 1);
  if (now - detailWheelTime > 200 || Math.sign(delta) !== Math.sign(detailWheelTotal)) detailWheelTotal = 0;
  detailWheelTime = now;
  detailWheelTotal += delta;
  if (Math.abs(detailWheelTotal) < 80) return;
  const direction = Math.sign(detailWheelTotal);
  detailWheelTotal = 0;
  if (!archiveJourney.begin(selected, direction, records.length, now)) return;
  setMode("archive");
  audio.play("back");
  syncJourney();
...
function toggleSaved() {
  ...
  try {
    localStorage.setItem("alphazero-rhine-saved-v2", JSON.stringify([...saved]));
  } catch {}
  $("#saved-count").textContent = String(saved.size).padStart(2, "0");
...
function renderDetail() {
  ...
  <dl class="metadata">${(r.resumeFields ?? []).map(field => `<div><dt>${escapeHtml(field.label)}</dt><dd>${escapeHtml(field.value)}</dd></div>`).join("")}</dl>
  <div class="detail-tabs" role="tablist"><button id="tab-overview" class="active" role="tab" aria-controls="tab-panel" aria-selected="true" data-tab="overview">01 <span>概述</span></button><button id="tab-notes" role="tab" aria-controls="tab-panel" aria-selected="false" data-tab="notes">02 <span>内容详情</span></button><button id="tab-history" role="tab" aria-controls="tab-panel" aria-selected="false" data-tab="history">03 <span>访问日志</span></button><i class="tab-indicator" aria-hidden="true"></i></div>
  <div id="tab-panel" class="tab-panel" role="tabpanel">${overview()}</div>
  <div class="detail-actions"><button class="solid-button" data-action="bookmark">${saved.has(r.id) ? "− REMOVE FROM SAVED" : "＋ SAVE ARCHIVE"}<span>${saved.has(r.id) ? "已收藏" : "收藏档案"}</span></button><a class="export-button" href="${assetUrl(`archives/ZIHAN-${r.id}.txt`)}" download="ZIHAN-${r.id}.txt" aria-label="导出 ${r.id} 档案">EXPORT <span>↓</span></a></div>
  <div class="record-links">${(r.links ?? []).map(link => `<a href="${escapeHtml(link.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(link.label)} ↗</a>`).join("")}</div>
  <div class="detail-footnote"><a href="${escapeHtml(r.source)}" target="_blank" rel="noopener">相关页面 ↗</a><span>${String(selected + 1).padStart(3, "0")} / ${String(records.length).padStart(3, "0")}</span></div>`;
...
function setTab(tab: string, sound = true) {
  ...
  $("#tab-panel").innerHTML =
    tab === "overview"
      ? overview()
      : tab === "notes"
        ? `<div class="panel-label">DETAIL NOTES / 内容详情</div><ol class="research-notes">${r.findings.map((f, i) => `<li><span>${String(i + 1).padStart(2, "0")}</span>${escapeHtml(f)}</li>`).join("")}</ol>`
        : `<div class="panel-label">ACCESS LOG / 本次访问</div>${accessLog
            .filter((entry) => entry.id === r.id)
            .slice(0, 4)
            .map(
              (entry) =>
                `<div class="log-row"><span>${entry.time}</span><span>ZIHAN DONG</span><b>READ AUTHORIZED</b></div>`,
            )
            .join(
              "",
            )}<p class="log-note">仅记录当前页面中的浏览操作，刷新后清空。</p>`;
...
function renderModal() {
  ...
  $("#modal-root").innerHTML =
    `<div class="modal-backdrop"><section class="terminal-modal ${modal === "settings" ? "settings-modal" : ""}" role="dialog" aria-modal="true" aria-label="${modal === "settings" ? "系统设置" : modal === "saved" ? "收藏档案" : "档案检索"}"><div class="modal-top"><span>ALPHAZERO / ${modal === "settings" ? "SYSTEM PREFERENCES" : "ARCHIVE DIRECTORY"}</span><button data-action="close-modal" aria-label="关闭窗口">CLOSE <span>×</span></button></div>${modal === "settings" ? settingsMarkup() : `<h2>${modal === "saved" ? "SAVED ARCHIVES" : "ARCHIVE INDEX"}<small>${modal === "saved" ? "收藏档案" : "个人档案检索"}</small></h2><div class="search-field"><span>⌕</span><input id="archive-search" type="search" autocomplete="off" placeholder="搜索姓名、经历或项目名称" aria-label="检索档案"/><span class="key">ESC</span></div><div class="category-filters">${categories.map((c, i) => `<button data-filter="${escapeHtml(c)}" class="${i === 0 ? "active" : ""}">${escapeHtml(c)}</button>`).join("")}</div><div class="result-header"><span>FILE / 档案</span><span>TOPIC / 归属</span><span>ACCESS</span></div><div id="search-results" class="search-results"></div><div class="modal-bottom"><span id="result-count"></span><span>PERSONAL ARCHIVE <i>●</i> CONNECTED</span></div>`}</section></div>`;
...
document.addEventListener("click", (e) => {
  ...
  if (!el) return;
  if (archiveJourney.active) cancelJourney();
...
document.addEventListener("keydown", (e) => {
  ...
  if (archiveJourney.active && e.key !== "Tab") { e.preventDefault(); return; }
  if (modal && e.key === "Tab") {
    const focusables = [
      ...$("#modal-root").querySelectorAll<HTMLElement>(
        'a[href],button,input:not(:disabled),select:not(:disabled),summary,[tabindex="0"]',
      ),
    ];
...
function bootFrame(t: number) {
  ...
  if (t >= 28.3) {
    step = "inspect";
    caption = t >= 29.3 ? "档案类型：个人主页" : "编号：X-001";
  }
  if (step !== lastStep) {
...
function scheduleFrame(delay = 0) {
  ...
  if (delay > 0) frameTimer = window.setTimeout(() => { frameRequest = requestAnimationFrame(frame); }, delay);
  else frameRequest = requestAnimationFrame(frame);
}
function wakeRendering() {
  renderBudget.wake(performance.now());
  // Do not repeatedly cancel an already scheduled animation frame during a drag.
  if (!frameRequest) scheduleFrame();
}
for (const event of ["pointerdown", "pointermove", "pointerup", "wheel", "keydown", "click", "input", "change", "resize"]) {
  window.addEventListener(event, wakeRendering, { passive: true });
}
document.addEventListener("visibilitychange", () => {
  if (document.hidden) { clearTimeout(frameTimer); cancelAnimationFrame(frameRequest); frameRequest = 0; }
  else { wakeRendering(); }
});
document.fonts.addEventListener("loadingdone", wakeRendering);
function frame(ms: number) {
  frameRequest = 0;
  if (document.hidden) return;
  const active = renderBudget.active(ms, mode === "boot" || archiveJourney.active, Boolean(modal));
  if (!active) {
    fps = 0;
    frameCount = 0;
    frameStart = ms;
    $("#three-scene").dataset.fps = "0";
    $("#three-scene").dataset.renderState = modal ? "covered" : "idle";
    $("#clock").textContent = new Date().toLocaleTimeString("en-GB");
    scheduleFrame(1000);
    return;
  }
  if (!renderBudget.take(ms)) { scheduleFrame(4); return; }
  $("#three-scene").dataset.renderState = "active";
  const time = ms / 1000;
  const cinema =
    mode === "boot" && ready
  ...
  viewer?.update(time);
  if (scene && archiveJourney.active) {
    const action = archiveJourney.advance(scene.navigationReadiness, ms);
    if (action === "move") select(archiveJourney.target);
    else if (action === "open") { setMode("detail"); audio.play("open"); }
    if (action) { renderBudget.wake(ms); syncJourney(); }
  }
...
async function start() {
  try {
    ...
    await Promise.all([
      ...
      document.fonts.load("400 20px MiSans", "正在加载请求已接收个人档案已就绪欢迎访问董梓涵的个人档案编号类型主页选择档案：0123456789 ZIHAN DONG"),
      document.fonts.load("600 20px MiSans", "ZIHAN DONG USTC PERSONAL OS"),
      document.fonts.load("700 20px MiSans", "ALPHAZERO ZIHAN DONG WELCOME TO PERSONAL ARCHIVE"),
    ]);
    ...
    savePrefs();
    ready = true;
    select(requestedIndex >= 0 ? requestedIndex : 0);
    if (entry) entry.ready();
    else completeStartup(false);
  } catch (error) {
    console.error(error);
    $("#loading").innerHTML =
      '<div class="error-state"><strong>CONNECTION INTERRUPTED</strong><p>三维档案资源未能载入。请确认浏览器已启用硬件加速，然后重新连接。</p><button onclick="location.reload()">重新加载 →</button><a class="fallback-reading" href="./profile.html">直接阅读个人概览 ↗</a></div>';
  }
}
function completeStartup(silent: boolean, skipBoot = false) {
  ...
  setMode("boot");
  if (skipBoot || reviewParams.get("scene") === "archive" || (prefs.reduced && !reviewParams.has("time"))) setMode("archive");
  if (requestedIndex >= 0 || reviewParams.get("scene") === "detail") setMode("detail");
  $("#stage").inert = false;
...
Object.assign(window, {
  rhine: {
    ...
    stats: () => ({
      ...scene?.getStats(),
      fps: Math.round(fps),
      renderedFrames: renderBudget.renderedFrames,
      renderState: $("#three-scene").dataset.renderState,
      mode,
      ready,
